<template>
  <div>
    <q-modal v-model="saveModalOpen">
      <q-modal-layout>
        ...
        <div>
          ...
          ><code v-html="formatHighlight(doc)" /></pre>
        </div>

        <div style="background-color:white">
          <q-item>
            <q-item-main
              label="Filename (leave empty to use document identifier):"
            >
            ...
            <q-item-side right>
              <q-btn
                color="primary"
                @click="saveDoc"
                aria-label="download metadata record"
              >
                <v-icon
                  aria-hidden="true"
    ...
    <q-modal
      v-model="loadModalOpen"
      :content-css="{ height: 'auto', 'min-width': '25vw' }"
    >
      <q-modal-layout>
        <div>
          <q-toolbar color="primary">
        ...
        <div v-if="selectedRecord">
          ...
          <pre
            v-else
            style="margin-top:0px;margin-bottom:0px"
          ><code v-html="formatHighlight(docToLoad.dataset[selectedRecord])" /></pre>
        </div>
        ...
        <div style="background-color:white">
          <q-item>
            ...
            <q-item-side left>
              ...
              >
                <v-icon aria-hidden="true" name="edit" scale="1.4" />
              </q-btn>
            </q-item-side>
          </q-item>
        </div>
...
export default {
  ...
  methods: {
    openLoadModal: function() {
      fetch('https://edg.epa.gov/data/public/epa-open-data-dcat-files.json')
        .then(response => response.json())
        .then(data => {
          console.log('DATA:', data)
          this.edgRoot = data.root
          this.availableRepos = data.files
            .map(repo => {
              return {
                value: repo.file,
                label: repo.title,
                epaOnly: repo.epaOnly || true
              }
            })
            .filter(repo => !repo.epaOnly || this.isEpaUser)
        })
        .then(() => {
          if (!this.availableRepos || !this.availableRepos.length)
            this.availableRepos = []
          this.availableRepos.unshift({
            value: 'local',
            label: '[From a local file...]'
          })
    ...
    loadDoc: function(e) {
      ...
      this.$emit('loadMd', {
        ...
        dataset: [config.clone(this.docToLoad.dataset[this.selectedRecord])]
      })
      this.closeLoadModal()
    },
    ...
    saveDoc: function() {
      ...
      this.$q.notify("Metadata document '" + this.filenameFull + "' downloaded")
    },
  ...
  computed: {
    ...
    filenameFull: {
      get: function() {
        return this.filenameInternal || this.doc.dataset[0].identifier + '.json'
      }
    }
  },
    ...
    selectedRepo: {
      handler: function() {
        if (this.edgRoot && this.selectedRepo && this.selectedRepo != 'local') {
          fetch(this.edgRoot + '/' + this.selectedRepo)
            .then(response => response.json())
            .then(data => {
              this.docToLoad = data
            })
    ...
    docToLoad: {
      handler: function(newDoc) {
        if (newDoc && newDoc.dataset)
          this.availableRecords = newDoc.dataset.map((record, index) => {
            return {
              value: index,
              label: record.title || 'Record #' + (index + 1)
            }
          })
      },
      immediate: true
